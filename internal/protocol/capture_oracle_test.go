...
// of the real formulas out of RØDE Connect.exe (docs/protocol.md), and the
...
//
// See docs/protocol.md for the capture methodology.
...
// TestTruncationMatchesCapturedIndices shows that indices truncate. These three UI
	...
	for _, c := range cases {
		...
		if le32(c.fn(c.labelled)) == c.captured {
			t.Errorf("%s(%v) now reproduces the capture at its labelled value; "+
				"move it into capturedVectors() and update docs/protocol.md",
				c.name, c.labelled)
		}
	}
}
